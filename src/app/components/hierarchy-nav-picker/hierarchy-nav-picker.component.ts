...
class NavHierarchyService implements SDSAutocompleteServiceInterface {
    ...
    getDataByText(currentItems: number, searchValue?: string): Observable<SDSHiercarchicalServiceResult> {
    	let result = this.hierarchyService.getMatch(this.root.name, this.includeChildren, currentItems, searchValue);
    	return result;
    }
}

class AutoHierarchyService implements SDSAutocompleteServiceInterface {
...
export class HierarchyNavPickerComponent implements OnInit {
  ...
  constructor(public service: HierarchyService) { 
     ...
     this.autocompleteService = new NavHierarchyService(this.breadcrumbs[this.breadcrumbs.length-1], service);
	...
	this.autocompleteSettings.autocompletePlaceHolderText = 'Enter an code or name';
  ...
  navigate(value) {
     if(this.breadcrumbs[this.breadcrumbs.length-1].id != value.currentTarget.name) {
  ...
  breadcrumbSelected(id) {
    if(id == '0000000') {
        ...
        this.autocompleteService.root = this.breadcrumbs[this.breadcrumbs.length-1];
        this.setLevelLabel();
    	return;
    }
