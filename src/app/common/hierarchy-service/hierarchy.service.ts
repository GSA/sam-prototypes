...
export class HierarchyService {
  ...
  match(item, searchValue) {
    ...
    if (
      item.org.aacCode &&
      item.org.aacCode.toLowerCase().startsWith(searchValue.toLowerCase())
    ) {
      return true;
    }
    if (
      item.org.cgac &&
      item.org.cgac.toLowerCase().startsWith(searchValue.toLowerCase())
    ) {
      return true;
    }
    if (
      item.org.fpdsCode &&
      item.org.fpdsCode.toLowerCase().startsWith(searchValue.toLowerCase())
    ) {
      return true;
    }
    if (
      item.org.shortName &&
      item.org.shortName.toLowerCase().startsWith(searchValue.toLowerCase())
    ) {
      return true;
    }
    if (
      item.org.name &&
      item.org.name.toLowerCase().indexOf(searchValue.toLowerCase()) !== -1
    ) {
      return true;
    }
    return false;
  ...
  getDataByText(
    ...
    let result = searchValue
      ? this.data.filter((element) => this.match(element, searchValue))
      : this.data;
    return of({
      items: result.slice(
        0 + currentItems,
        30 + currentItems < result.length ? 30 + currentItems : result.length
      ),
